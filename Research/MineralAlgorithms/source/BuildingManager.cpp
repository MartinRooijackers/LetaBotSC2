...
void BuildingManager::assignWorkersToUnassignedBuildings()
{
    ...
    for (Building & b : m_buildings)
    {
		...
		const sc2::Unit * builderUnit = m_bot.m_baseMan.getBuilder();

		//std::cerr << "build unit assinged?";
		//BOT_ASSERT(builderUnit, "null builder unit");
		//std::cerr << builderUnit->tag;

        b.builderUnit = builderUnit;
        if (!b.builderUnit)
        {
			std::cerr << "no build unit found";
			BOT_ASSERT(builderUnit, "null builder unit");
            continue;
        }
...
void BuildingManager::constructAssignedBuildings()
{
    for (auto & b : m_buildings)
    {
        ...
        {
            BOT_ASSERT(builderUnit, "null builder unit");
            isConstructing = (builderUnit->orders.size() > 0) 
				&& (builderUnit->orders[0].ability_id == buildAbility);
			//sc2::ABILITY_ID::build
        }
